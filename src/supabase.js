...
export const SUPABASE_PUBLISHABLE_KEY =
  "sb_publishable_8Nlj9kEnB-Rj_nsxnv_YQg_poT8S9GH";
export const GOOGLE_WEB_CLIENT_ID =
  "376420281293-agemocfufrdhc4146oomksnpjf9mkngu.apps.googleusercontent.com";
...
function loadGoogleIdentity() {
  ...
  googleIdentityPromise = new Promise((resolve, reject) => {
    const existing = document.querySelector(
      "script[data-usecrm-google-identity]",
    );
  ...
  try {
    ...
    return await new Promise((resolve) => {
      ...
      overlay.style.cssText =
        "position:fixed;inset:0;z-index:99999;display:grid;place-items:center;background:rgba(2,10,5,.72);backdrop-filter:blur(8px);padding:20px;";

      const card = document.createElement("div");
      card.style.cssText =
        'width:min(390px,100%);background:#0b1710;border:1px solid rgba(255,255,255,.12);border-radius:22px;padding:28px;box-shadow:0 28px 80px rgba(0,0,0,.5);color:#fff;font-family:Inter,ui-sans-serif,system-ui,-apple-system,BlinkMacSystemFont,"Segoe UI",sans-serif;';
      ...
      copy.style.cssText =
        "font-size:14px;line-height:1.5;color:rgba(255,255,255,.68);margin-bottom:22px;";
      ...
      cancel.style.cssText =
        "width:100%;margin-top:14px;padding:11px 14px;border-radius:12px;border:1px solid rgba(255,255,255,.12);background:transparent;color:rgba(255,255,255,.72);cursor:pointer;font:inherit;";
      ...
      cancel.addEventListener("click", () =>
        finish({ data: null, error: new Error("Google sign-in cancelled.") }),
      );
      ...
      google.accounts.id.initialize({
        ...
        callback: async (response) => {
          if (!response?.credential) {
            finish({
              data: null,
              error: new Error("Google did not return a sign-in credential."),
            });
            return;
          }
