...
import {
  Activity, BadgePercent, BarChart3, BellRing, Boxes, Check, ChevronDown, CircleDollarSign,
...
function App() {
  ...
  async function update(table, id, patch, label='Updated') {
    if (demo) {
      const key = Object.keys({crm_orders:'orders',crm_order_forms:'forms',crm_followups:'followups',crm_deliveries:'deliveries',crm_coupons:'coupons',crm_automations:'automations'}).find(() => false)
      setData(prev => ({ ...prev, orders: table==='crm_orders' ? prev.orders.map(x=>x.id===id?{...x,...patch}:x) : prev.orders, forms:table==='crm_order_forms'?prev.forms.map(x=>x.id===id?{...x,...patch}:x):prev.forms }))
  ...
  async function addProduct() {
    ...
    if (demo) {
      const pid=crypto.randomUUID(), vid=crypto.randomUUID(); setData(p=>({...p,products:[{id:pid,name,sku,active:true,track_inventory:false,created_at:new Date().toISOString()},...p.products],variants:[{id:vid,product_id:pid,name:'Default',price,stock_qty:0,active:true},...p.variants]})); notify('Product added in preview mode'); return
    }
  ...
  async function addAgent() {
    ...
    const phone=window.prompt('Phone number','')||null; const whatsapp=window.prompt('WhatsApp number',phone||'')||null; const states=(window.prompt('States covered, comma separated','Lagos')||'').split(',').map(x=>x.trim()).filter(Boolean)
    const row={name,phone,whatsapp,states_covered:states,status:'active',commission_type:'none',commission_value:0}
    if(demo){setData(p=>({...p,agents:[{id:crypto.randomUUID(),...row},...p.agents]}));notify('Agent added in preview mode');return} await insert('crm_agents',row,'Agent added')
  }

  async function addCoupon() {
    const code=(window.prompt('Coupon code','SAVE10')||'').trim().toUpperCase(); if(!code)return
    const type=window.confirm('OK = percentage discount. Cancel = fixed amount.')?'percent':'fixed'; const value=Number(window.prompt(type==='percent'?'Percentage':'Discount amount','10')); if(!Number.isFinite(value))return
    const row={code,discount_type:type,discount_value:value,active:true,uses_count:0}; if(demo){setData(p=>({...p,coupons:[{id:crypto.randomUUID(),...row},...p.coupons]}));notify('Coupon added in preview mode');return} await insert('crm_coupons',row,'Coupon created')
  }

  async function addForm() {
    const title=window.prompt('Order form title','New Order Form'); if(!title)return
    const slug=slugify(window.prompt('Public URL slug',slugify(title))||slugify(title)); const site=data.sites[0]
    ...
    for(let i=0;i<defaults.length;i++){const [field_key,label,field_type,required]=defaults[i];await supabase.from('crm_form_fields').insert({workspace_id:workspace.id,form_id:form.id,field_key,label,field_type,required,sort_order:i+1})}
    await loadAll()
  }

  async function addField(form) {
    const label=window.prompt('Field label','Delivery date'); if(!label)return
    const key=slugify(window.prompt('Field key',slugify(label))||slugify(label)).replace(/-/g,'_'); const type=window.prompt('Type: text, tel, email, textarea, select, radio, checkbox, date, number','text')||'text'
  ...
  async function attachVariant(form, bump=false) {
    ...
    const idx=Number(window.prompt(`Choose package:\n${list}`,'1'))-1; const variant=data.variants[idx]; if(!variant)return
    if(bump){const title=window.prompt('Order bump headline','Add this special offer')||'Special offer';const price=Number(window.prompt('Bump price',variant.price));await insert('crm_order_bumps',{form_id:form.id,variant_id:variant.id,title,offer_price:price,active:true},'Order bump added')}
    else await insert('crm_form_products',{form_id:form.id,variant_id:variant.id,default_quantity:1,active:true},'Package attached')
  }

  async function addFollowup() {
    if(!data.buyers.length)return notify('No customers yet.')
    const buyer=data.buyers[0]; const channel=window.prompt('Channel: call, whatsapp, sms, email','whatsapp')||'whatsapp'; const note=window.prompt(`Follow-up note for ${buyer.name}`,'Confirm order / recover checkout')||''
    const due=new Date(Date.now()+30*60000).toISOString(); const row={buyer_id:buyer.id,channel,due_at:due,status:'pending',note}; if(demo){setData(p=>({...p,followups:[{id:crypto.randomUUID(),...row},...p.followups]}));notify('Follow-up added in preview mode');return} await insert('crm_followups',row,'Follow-up scheduled')
  }
  ...
  async function addDelivery() {
    ...
    const provider=window.prompt('Delivery provider','In-house')||'In-house'; const rider_name=window.prompt('Rider name (optional)','')||null; const fee=Number(window.prompt('Delivery fee','0')||0)
    const row={order_id:order.id,provider,rider_name,fee,status:'scheduled',scheduled_for:new Date(Date.now()+3600000).toISOString()}; if(demo){setData(p=>({...p,deliveries:[{id:crypto.randomUUID(),...row},...p.deliveries]}));notify('Delivery scheduled in preview mode');return} await insert('crm_deliveries',row,'Delivery scheduled'); await update('crm_orders',order.id,{status:'scheduled',fulfillment_status:'scheduled',delivery_fee:fee,total:Number(order.total||0)+fee},'Order scheduled')
  }

  async function addAutomation() {
    const name=window.prompt('Automation name','Recover hot abandoned checkout'); if(!name)return
    const trigger=window.prompt('Trigger event','visitor.abandoned')||'visitor.abandoned'; const row={name,trigger_event:trigger,conditions:{intent_score_gte:70},actions:[{type:'create_followup',channel:'whatsapp',delay_minutes:10}],active:true}
    if(demo){setData(p=>({...p,automations:[{id:crypto.randomUUID(),...row},...p.automations]}));notify('Automation added in preview mode');return} await insert('crm_automations',row,'Automation created')
  }
...
function Overview({ctx:{data}}){
  const delivered=data.orders.filter(o=>o.status==='delivered'), revenue=delivered.reduce((s,o)=>s+Number(o.total||0),0), hot=data.visitors.filter(v=>v.intent_score>=70&&v.status!=='submitted')
  ...
  <div className="split-grid"><Panel title="Order pipeline" action="Live"><Pipeline orders={data.orders}/></Panel><Panel title="Highest intent visitors" action={`${hot.length} recoverable`}><div className="stack-list">{(hot.length?hot:data.visitors).slice(0,5).map(v=><VisitorRow v={v} key={v.id}/>)||<Empty/>}</div></Panel></div>
...
function Integrations({ctx:{data,workspace,demo,insert,notify}}){const site=data.sites[0];const snippet=site?`<script>\nwindow.useCRM={siteKey:"${site.public_key}"};\n/* send page/form events to crm_track_event */\n<\/script>`:'Create a site connection first.';async function addSite(){const domain=window.prompt('Website domain','your-store.com');if(!domain)return;const row={name:domain,domain,active:true};if(demo){notify('Site added in preview mode');return}await insert('crm_sites',row,'Website connected')};async function addWebhook(){const url=window.prompt('Webhook URL','https://example.com/webhooks/usecrm');if(!url)return;await insert('crm_webhooks',{url,events:['order.created','order.updated','visitor.identified'],active:true},'Webhook added')}
...
function SettingsPage({ctx:{workspace,data,role,demo}}){return <><PageHead title="Settings" copy="Workspace, sites, access and checkout defaults."/><div className="settings-grid"><Panel title="Workspace"><dl className="details"><div><dt>Name</dt><dd>{workspace?.name}</dd></div><div><dt>Currency</dt><dd>{workspace?.currency||'NGN'}</dd></div><div><dt>Timezone</dt><dd>{workspace?.timezone||'Africa/Lagos'}</dd></div><div><dt>Your role</dt><dd>{role}</dd></div></dl></Panel><Panel title="Authentication"><div className="auth-status"><div className="provider enabled"><span>G</span><div><strong>Google OAuth</strong><small>Frontend integration ready</small></div><Status value="configured in app"/></div><div className="provider"><span>◎</span><div><strong>ChatGPT</strong><small>Keep the existing ChatGPT Sites login during migration; standalone provider access depends on OpenAI partner onboarding.</small></div></div></div></Panel><Panel title="Multi-site"><div className="big-stat">{data.sites.length}<span>connected websites</span></div><p className="muted">Each site has its own scoped tracking key while sharing customers, orders and analytics inside this workspace.</p></Panel><Panel title="Environment"><dl className="details"><div><dt>Mode</dt><dd>{demo?'Preview':'Live Supabase'}</dd></div><div><dt>Tenant RLS</dt><dd>Enabled</dd></div><div><dt>Legacy tracker</dt><dd>Compatible</dd></div></dl></Panel></div></>}

function PublicOrderForm({slug}){
  const params=new URLSearchParams(window.location.search), siteKey=params.get('site_key'); const [form,setForm]=useState(null),[error,setError]=useState(null),[values,setValues]=useState({}),[qty,setQty]=useState({}),[bumps,setBumps]=useState([]),[coupon,setCoupon]=useState(''),[submitting,setSubmitting]=useState(false),[done,setDone]=useState(null);const visitor=useRef(localStorage.getItem('usecrm_visitor_id')||crypto.randomUUID()),session=useRef(sessionStorage.getItem('usecrm_session_id')||crypto.randomUUID()),timer=useRef(null)
  ...
  async function load(){if(!siteKey){setError('This order-form link is missing its site key.');return}const {data,error}=await supabase.rpc('crm_get_public_form',{p_site_key:siteKey,p_slug:slug});if(error||!data){setError(error?.message||'Order form not found.');return}setForm(data);const initial={};(data.products||[]).forEach((p,i)=>initial[p.variant_id]=i===0?(p.default_quantity||1):0);setQty(initial);track('page_view',{}) }
  async function track(event,next){if(!siteKey)return;const fields=Object.keys(next||values).filter(k=>String((next||values)[k]??'').trim());await supabase.rpc('crm_track_event',{p_site_key:siteKey,p_visitor_id:visitor.current,p_session_id:session.current,p_page_url:window.location.href,p_form_name:slug,p_event_type:event,p_fields_touched:fields,p_form_data:{...(next||values),referrer:document.referrer||null,utm_source:params.get('utm_source'),utm_medium:params.get('utm_medium'),utm_campaign:params.get('utm_campaign')}})}
  function change(key,value){const next={...values,[key]:value};setValues(next);clearTimeout(timer.current);timer.current=setTimeout(()=>track('form_progress',next),500)}
  async function submit(e){e.preventDefault();const items=Object.entries(qty).filter(([,q])=>Number(q)>0).map(([variant_id,quantity])=>({variant_id,quantity:Number(quantity)}));if(!items.length)return setError('Select at least one package.');setSubmitting(true);const payload={customer:{name:values.name,phone:values.phone,email:values.email,whatsapp:values.whatsapp,address:values.address,city:values.city,state:values.state,country:values.country||'Nigeria'},items,bumps,coupon_code:coupon,payment_method:values.payment_method,payment_readiness:values.payment_readiness,delivery_date:values.delivery_date,delivery_window:values.delivery_window,delivery_address:values.address,delivery_city:values.city,delivery_state:values.state,answers:values,attribution:{utm_source:params.get('utm_source'),utm_medium:params.get('utm_medium'),utm_campaign:params.get('utm_campaign')}};const {data,error}=await supabase.rpc('crm_submit_public_order',{p_site_key:siteKey,p_form_slug:slug,p_payload:payload});setSubmitting(false);if(error){setError(error.message);return}await track('form_submitted',values);setDone(data)}
  if(error&&!form)return <PublicShell><div className="public-message"><X size={28}/><h2>Order form unavailable</h2><p>{error}</p></div></PublicShell>; if(!form)return <PublicShell><LoadingCard/></PublicShell>; if(done)return <PublicShell><div className="public-message success"><Check size={30}/><h2>{done.success_message||'Order received'}</h2><p>Your order number is <strong>{done.order_number}</strong>.</p><div className="public-total"><span>Total</span><strong>{money(done.total)}</strong></div></div></PublicShell>
...
function Field({field,value,onChange}){const common={value,onChange:e=>onChange(e.target.type==='checkbox'?e.target.checked:e.target.value),required:field.required,placeholder:field.placeholder||field.label};if(field.type==='textarea')return <label className="field"><span>{field.label}{field.required&&' *'}</span><textarea {...common}/></label>;if(['select','radio'].includes(field.type)){const opts=Array.isArray(field.options)?field.options:[];return <label className="field"><span>{field.label}{field.required&&' *'}</span><select {...common}><option value="">Select…</option>{opts.map(o=><option key={typeof o==='string'?o:o.value} value={typeof o==='string'?o:o.value}>{typeof o==='string'?o:o.label}</option>)}</select></label>}if(field.type==='checkbox')return <label className="check-field"><input type="checkbox" checked={Boolean(value)} onChange={common.onChange}/><span>{field.label}</span></label>;return <label className="field"><span>{field.label}{field.required&&' *'}</span><input type={field.type||'text'} {...common}/></label>}
